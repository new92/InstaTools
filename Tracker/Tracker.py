...
def main():
    ...
    if num == 1:
        ...
        name = 'additions.txt'
        if num == 1:
            print(f"{GREEN}[✓] Tracker started successfully !")
            ...
            if len(followers_af) - len(followers_bef) == 1:
                users = [username for username in followers_af if username not in followers_bef]
            ...
            if len(followings_af) - len(followings_bef) == 1:
                users = [username for username in followings_af if username not in followers_bef]
            ...
            if followers_af != followers_bef:
                ...
                if len(followings_af) - len(followings_bef) == 1:
                    users = [username for username in followings_af if username not in followings_bef]
                    messagebox.showinfo('following addition'.upper(),f'{user} started following \n▶ {users[0]}')
                else:
                    f = open(name,'a')
                    count = 0
